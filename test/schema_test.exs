defmodule ArcTest.Ecto.Schema do
  ...
  test_with_mock "converts string keys", DummyDefinition, [store: fn({%{__struct__: Plug.Upload, path: "/path/to/my/file.png", filename: "file.png"}, %TestUser{}}) -> {:error, :invalid_file} end] do
    upload = build_upload("/path/to/my/file.png")
    TestUser.changeset(%TestUser{}, %{"avatar" => upload})
    assert called DummyDefinition.store({upload, %TestUser{}})
  end
